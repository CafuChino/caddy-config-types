...
export default async function GeneratorModule (ctx: GeneratorCtx, path: string, key:string, s: CaddyDocConfigStructureModule) {
  let result = '';
  ...
  if (!namespace) {
    console.warn(`Namespace "${module_namespace}" required by module ${path}${key} not found`);
    // type of the key defaults to any as a fallback
    return `  ${key}: any;\n`;
  }
  ...
  console.log(`[Module ${path}${key}] Generating namespace interface ${namespaceInterfaceName}`);
  ...
  for (const item of namespace) {
    try {
      const lastPath = path.split('/').filter(item => item).pop();
      const docApiPath = lastPath === key ? join(path, item.name, '/') : join(path, key, item.name, '/');
      ...
      console.error(`[Module ${path}${key}] Error fetching interface ${item.name}:`, e);
      const name = getInterfaceName(item.name);
      interfaceNames.add(name);
      ctx.interfaceMap.set(name, `export type ${name} = any;`);
      console.error(`[Module ${path}${key}] Fallback: Generating interface ${name} as any`);
    }
  }

  // Add type union
  const typeUnion =  `export type ${namespaceInterfaceName} = ${Array.from(interfaceNames).filter(item => item !== namespaceInterfaceName).join(' | ') || 'any'};\n`;
